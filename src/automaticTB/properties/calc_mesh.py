...
def calculate_e_c2_using_ibz(
    ...
    Note that the orbital component may not be simply the same for
    equivalent k points. But should be the same if we do not distinguish
    different atoms
    """
    cell = (tb.cell, tb.positions, tb.types)
    sym_data = spglib.get_symmetry_dataset(cell, symprec=parameters.spgtol)
    ...
    e, c = tb.solveE_at_ks(unique_kpoints)
    c2 = np.abs(c)**2 # (nk, norb, nbnd)
    _, nbnd = e.shape

    full_e = np.zeros((nk, nbnd))
    full_c2 = np.zeros((nk, nbnd, nbnd))
    for i, (eqk, ri) in enumerate(zip(symmetry_eq, rotation_index)):
        which_k = rev[eqk]
        full_e[i, :] = e[which_k,:]
        full_c2[i,:,:] = c2[which_k,:,:]

    return full_e, full_c2