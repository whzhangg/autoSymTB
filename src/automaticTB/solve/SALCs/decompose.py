...
from .vectorspace import OrbitalVectorSpace, NamedLC, IrrepSymbol
...
def decompose_vectorspace_to_namedLC(
    vectorspace: OrbitalVectorSpace, group: ssym.SiteSymmetryGroup
) -> typing.List[NamedLC]:
...
def decompose_vectorspace_to_namedLC_sphericalsymmetric(
        vectorspace: OrbitalVectorSpace) -> typing.List[NamedLC]:
    """
    this function treat the special case of spherical symmetric group
    """
    nlms = []
    nlms += [orb.sh_list for orb in vectorspace.orbital_list]
    vectors = vectorspace.get_a_set_of_basis()
    if len(nlms) != len(vectors):
        print("decompose spherical symmetric vectorspace, something wrong")
        raise

    named_lcs = []
    for nlm, v in zip(nlms, vectors):
        name = f"{nlm.n}-{nlm.l}^1->{nlm.m}"

        named_lcs.append(
            NamedLC(IrrepSymbol.from_str(name), vectorspace.aos, v)
        )
...
def _decompose_onelevel_for_irrep(
    vectorspace: OrbitalVectorSpace, vectors: typing.List[np.ndarray], operations: typing.List[CartesianOrbitalRotation], irrep: IrreducibleRep
    ...
    transformed_LCs = []
    for v in vectors:
        sum_coefficients = np.zeros_like(v)
